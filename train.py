...
def load_dataset(dataset_name):
    ...
    if dataset_name == 'indian_pines':
        ...
        data = loadmat(data_path)['salinasA']
        labels = loadmat(gt_path)['salinasA_gt']
        # Transpose if needed
        if len(data.shape) == 3 and data.shape[0] < data.shape[1] and data.shape[0] < data.shape[2]:
            data = data.transpose(1, 2, 0)
        # Crop data to match labels
        data = data[:83, :86, :]
    ...
    # The data should already be in (H, W, C) format for hyperspectral images
    # Only transpose if the data is in (C, H, W) format (which would have C as first dimension)
    if len(data.shape) == 3 and data.shape[0] < data.shape[1] and data.shape[0] < data.shape[2]:
        print(f"Data appears to be in (C, H, W) format, transposing to (H, W, C)")
        data = data.transpose(1, 2, 0)
...
def extract_patches(data, labels, patch_size=15):
    ...
    """
    print(f"extract_patches input - data shape: {data.shape}, labels shape: {labels.shape}")
    ...
    if data.shape[0] != labels.shape[0] or data.shape[1] != labels.shape[1]:
        ...
        # For hyperspectral data, the spectral dimension should be the largest
        # Find the spectral dimension (should be 200 for raw data, 30 after PCA)
        spec_dim = max(data.shape)  # Spectral dimension should be largest
        
        # Get current order of dimensions
        dims = list(range(len(data.shape)))
        spec_dim_idx = data.shape.index(spec_dim)
        
        # Move spectral dimension to last position (H, W, C)
...
def apply_pca(data, n_components=30):
    ...
    Args:
        data: Input hyperspectral data (H, W, C)
    ...
    h, w, c = data.shape  # Always use original spatial dimensions
    data_2d = data.reshape(-1, c)  # (H*W, C)
    ...
    data_2d_pca = pca.fit_transform(data_2d_scaled)  # (H*W, n_components)
    
    # Reshape back to image format
    data_pca = data_2d_pca.reshape(h, w, n_components)  # (H, W, n_components)
